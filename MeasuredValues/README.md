...
## Maximum Capacity is not derivable from these dumps

The Maximum Capacity percentage System Settings shows cannot be computed from any capacity value here.
Four machines have been measured against their System Settings reading, and neither
`AppleRawMaxCapacity / DesignCapacity` — what `BatteryRepository` reports — nor
`NominalChargeCapacity / DesignCapacity` reproduces any of them, with no meaningful accuracy difference
between the two. macOS is most likely displaying a smoothed or cached figure rather than a live gauge
reading.

`BatteryRepository` therefore keeps the key it has. Picking a different one would only change which
machines look wrong.
