...
- **`maxCapacity` does not reproduce System Settings.** Three machines have been measured against their System Settings reading — M1 Pro 91%, M4 Pro 97%, M2 Air 100% — and the current `AppleRawMaxCapacity / DesignCapacity` reproduces none of them (93.89, 92.77, 97.02). For the first two the numerator would have to land in 5498-5559 and 6030-6093, and no key in either dump holds a value in range, nor does any ratio of two keys. macOS is most likely displaying a smoothed or cached figure rather than a live gauge reading. **`NominalChargeCapacity` is currently ahead on that evidence** — it reproduces the M2 Air reading and is the closer of the two on two of three — but not by enough to act on: both keys overshoot the M1 Pro badly and neither explains it, and nominal sits a near-constant +2.4 to +2.8 points above raw on all nine dumps, a constant shift that cannot fix an error changing sign. The reading that would settle it is the M3 Air on macOS 27 — an interior point where the two keys are 2.8 apart, unlike the M2 Air which is pinned at the ceiling. `MeasuredValues/README.md` holds the table. (`AppleRawMaxCapacity` equals `BatteryData.FccComp2` on eight of nine dumps — same quantity under two names.)
- **Do not switch the capacity key on reasoning alone.** It has repeatedly drawn "the battery info is wrong" reports in the past, and that is unsurprising: no candidate matches System Settings, so changing it swaps which users notice rather than fixing anything. Nor is there a structural argument either way — `NominalChargeCapacity` sits at the top level pre-27 and under `BatteryData` on 27, exactly like the current pair, so the `if #available` stays regardless. What would justify a switch is more dump-plus-System-Settings pairs, especially interior ones.
